...
public class LineTcpReceiverTest extends AbstractLineTcpReceiverTest {
    ...
    @Test
    public void testCreationFailsForNonPartitionedTableWithWAL() throws Exception {
        // WAL is not supported on non-partitioned tables
        ...
        runInContext((receiver) -> {
            ...
            final String lineData =
                    tableNonPartitioned + " windspeed=2.0 631150000000000000\n" +
            ...
            // Verify that the partitioned table data has landed, but non-partitioned table is absent.
            mayDrainWalQueue();
            ...
            int status = engine.getStatus(AllowAllCairoSecurityContext.INSTANCE, path,
                    tableNonPartitioned, 0, tableNonPartitioned.length());
            Assert.assertEquals(TableUtils.TABLE_DOES_NOT_EXIST, status);
        });
    }
